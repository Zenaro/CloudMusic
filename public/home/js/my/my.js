define ( function ( require, exports, module ) {
	...
	My.prototype._init = function () {
		...
		if ( !cookie('unique') || cookie('unique') == '' ) {
			...
			$.post('../../phpCtrl/myMusic.php', {uid: cookie('unique')}, function (res) {
			...
			$.get('../../phpCtrl/myFriend.php', {uid: cookie('unique')}, function (res) {
				var json = $.parseJSON(res);
				var html = '';
				$.each(json, function ( index, value ) {
	...
	My.prototype._bind = function () {
		...
		$('.wrap-in').on({
			...
			if( !!cookie('unique') ) {
				...
				$.get('../../phpCtrl/delMusic.php', {'uid':cookie('unique'), 'mid':delID} );
				$(obj).remove();
			}
