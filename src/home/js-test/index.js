...
define( function( require, exports, module ) {
	...
	Index.prototype._load = function() {
		...
		$.get('../controller/getNews.php', function(result) {
			var json = $.parseJSON(result);
			var html = '';
			$.each(json, function (index, value) {
				html += '<li><a href="javascript:;">'+value.title+'</a><span>'+value.pubDate.substring(5,10)+'</span></li>';
			});	
		...
		function rank(i) {
			...
			$.post('../controller/getRank.php', {data: i}, function(result) {
				...
				var html = '';
				$.each(json, function (index, value) {
					html += '<dd data-id="' + value.id + '"><span>' + (index+1) + '</span><a href="javascript:;">'+value.name+'</a><div class="dd-oper"><a href="javascript:;" class="icon-play"></a><a href="javascript:;" class="icon-add"></a><a href="javascript:;" class="icon-store"></a></div></dd>';
				});	
	...
	Index.prototype._bind = function() {
		...
		$('.wrap .main-rank').on('click', this.rankALLPlay, function () {
			...
			self._appendMusic( addID, true );
			
		}).on('click', this.rankLIAdd, function() {

			var addID = $(this).parents('dd').attr('data-id');
			self._appendMusic( addID );	
		...
		function rankALLPLAY (obj, isPlay) {
			var objLI = $(obj).parents('dl').children('dd');
			for (var i = 0, length = $(objLI).length; i < length; i++) {
				var addID = $(objLI).eq(i).attr('data-id');
				if (i == 0 && isPlay) {
					self._appendMusic( addID, true );
				} else {
					self._appendMusic( addID );
	...
	Index.prototype._appendMusic = function ( addID, isPlay ) {
		...
		if (i == 0) {
			...
			for (  ; i >= 0; i--) {
				...
				if ( addID == existID && isPlay == true) { 
					var Music = require('./music');
					var M = new Music();
					M._reload( addID );
					break;

				} else if ( addID == existID ) {
					break;
		...
		function getSrc ( addID, booln ) {
			$.get('../controller/getMInfo.php', {id : addID}, function (result) {
				var json = $.parseJSON(result);
				var html = '';

				$.each( json, function ( index, value ) {
					html = '<li data-id="' + value.id + '"><div class="col col-1">' + value.name + '</div><div class="col col-2"><a href="javascript:;" class="icn-col" title="收藏"></a><a href="javascript:;" class="icn-dwn" title="下载"></a><a href="javascript:;" class="icn-del" title="删除"></a></div><div class="col col-3">' + value.master + '</div><div class="col col-4">03:23</div></li>'
				});
